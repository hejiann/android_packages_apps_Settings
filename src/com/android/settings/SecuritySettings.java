 ...
 */
public class SecuritySettings extends SettingsPreferenceFragment implements
	...
	private PreferenceScreen createPreferenceHierarchy() {
		...
		mLockAfter = (ListPreference) root
				.findPreference(KEY_LOCK_AFTER_TIMEOUT);
		if (mLockAfter != null) {
			...
			mSlideLockScreenOffDelay.setValue(String
					.valueOf(slideScreenOffDelay));
		...
		if (isCmSecurity) {
			// visible pattern
			mVisiblePattern = (CheckBoxPreference) root
					.findPreference(KEY_VISIBLE_PATTERN);

			// lock instantly on power key press
			mPowerButtonInstantlyLocks = (CheckBoxPreference) root
					.findPreference(KEY_POWER_INSTANTLY_LOCKS);
			...
			mMenuUnlock = (CheckBoxPreference) root
					.findPreference(MENU_UNLOCK_PREF);
		...
		mBiometricWeakLiveliness = (CheckBoxPreference) root
				.findPreference(KEY_BIOMETRIC_WEAK_LIVELINESS);
		...
		mVisibleErrorPattern = (CheckBoxPreference) root
				.findPreference(KEY_VISIBLE_ERROR_PATTERN);

		// visible dots
		mVisibleDots = (CheckBoxPreference) root
				.findPreference(KEY_VISIBLE_DOTS);
		...
		if (resid == R.xml.security_settings_biometric_weak
			...
			PreferenceGroup securityCategory = (PreferenceGroup) root
					.findPreference(KEY_SECURITY_CATEGORY);
		...
		if (!((Vibrator) getSystemService(Context.VIBRATOR_SERVICE))
				.hasVibrator()) {
			PreferenceGroup securityCategory = (PreferenceGroup) root
					.findPreference(KEY_SECURITY_CATEGORY);
		...
		if (!isCmSecurity) {
			...
			if (!tm.hasIccCard()) {
				...
				if ((TelephonyManager.getDefault().getSimState() == TelephonyManager.SIM_STATE_ABSENT)
						|| (TelephonyManager.getDefault().getSimState() == TelephonyManager.SIM_STATE_UNKNOWN)) {
					root.findPreference(KEY_SIM_LOCK).setEnabled(false);
				}
			}

			// Show password
			mShowPassword = (CheckBoxPreference) root
					.findPreference(KEY_SHOW_PASSWORD);
	...
	private void warnAppInstallation() {
		...
		mWarnInstallApps = new AlertDialog.Builder(getActivity())
				...
				.setMessage(
						getResources().getString(R.string.install_all_warning))
	...
	private void updateSlideAfterTimeoutSummary() {
		...
		long currentTimeout = Settings.System.getInt(getActivity()
				.getApplicationContext().getContentResolver(),
				Settings.System.SCREEN_LOCK_SLIDE_TIMEOUT_DELAY, 5000);
	...
	private void setupLockAfterPreference() {
		...
		final long adminTimeout = (mDPM != null ? mDPM
				.getMaximumTimeToLock(null) : 0);
		final long displayTimeout = Math.max(0, Settings.System.getInt(
	...
	private void updateLockAfterPreferenceSummary() {
		...
		mLockAfter.setSummary(getString(R.string.lock_after_timeout_summary,
				entries[best]));
	}
	...
	private void disableUnusableTimeouts(long maxTimeout) {
		...
		if (revisedEntries.size() != entries.length
			...
			mLockAfter.setEntries(revisedEntries
					.toArray(new CharSequence[revisedEntries.size()]));
			mLockAfter.setEntryValues(revisedValues
	...
	public void onResume() {
		...
		final LockPatternUtils lockPatternUtils = mChooseLockSettingsHelper
				.utils();
		if (mBiometricWeakLiveliness != null) {
			mBiometricWeakLiveliness.setChecked(lockPatternUtils
					.isBiometricWeakLivelinessEnabled());
		}
		if (mVisiblePattern != null) {
			mVisiblePattern.setChecked(lockPatternUtils
		...
		if (mTactileFeedback != null) {
			mTactileFeedback.setChecked(lockPatternUtils
					.isTactileFeedbackEnabled());
		}
		if (mPowerButtonInstantlyLocks != null) {
			mPowerButtonInstantlyLocks.setChecked(lockPatternUtils
	...
	public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
		...
		if (KEY_UNLOCK_SET_OR_CHANGE.equals(key)) {
			...
			if (!helper.launchConfirmationActivity(
					CONFIRM_EXISTING_FOR_BIOMETRIC_WEAK_IMPROVE_REQUEST, null,
					null)) {
			...
			if (isToggled(preference)) {
				...
				ChooseLockSettingsHelper helper = new ChooseLockSettingsHelper(
						this.getActivity(), this);
				if (!helper.launchConfirmationActivity(
						CONFIRM_EXISTING_FOR_BIOMETRIC_WEAK_LIVELINESS_OFF,
						null, null)) {
			...
			lockPatternUtils
					.setPowerButtonInstantlyLocks(isToggled(preference));
	...
	public boolean onPreferenceChange(Preference preference, Object value) {
		if (preference == mLockAfter) {
			...
			try {
				...
				Log.e("SecuritySettings",
						"could not persist lockAfter timeout setting", e);
			}
			updateLockAfterPreferenceSummary();
			checkPowerInstantLockDependency();
			...
			Settings.System.putInt(getActivity().getApplicationContext()
					.getContentResolver(),
					Settings.System.SCREEN_LOCK_SLIDE_SCREENOFF_DELAY,
					slideScreenOffDelay);
			updateSlideAfterScreenOffSummary();
	...
	public void startBiometricWeakImprove() {
		...
		intent.setClassName("com.android.facelock",
				"com.android.facelock.AddToSetup");
